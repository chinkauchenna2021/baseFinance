...
contract BorrowToken is ERC721, ERC721Burnable, Ownable {
   ...
   event SetDeadline (uint newDeadine);
   event SetBorrowPercent(uint _newPercentage);
   event BorrowAmount(uint _amountPaidByContract , uint _amountPaidByUser);
   event RepayBorrow(uint _amountPaidByContract , uint _amountPaidByUsers);
...
//   usersInfo
  for(uint i = 0 ; i < usersInfo.length ; i++){
